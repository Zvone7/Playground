...
namespace Playground.aoc2023.t9.part1;

public class Task9Part1
{
    public void Main()
    {
        var fileName = "1.txt";
        ...
        CalcPart(lines, false);// 114 && 2008960228
    }
    ...
    private void CalcPart(String[] lines, Boolean print = false)
    {
        ...
        foreach (var sequence in input.Sequences)
        {
            CalculateAllLastNumbers(sequence);
        }

        PrintHelp(input, print);


        Console.WriteLine($"Total: {input.Sequences.Sum(x => x.Main.Last())}");
    ...
    private void CalculateAllSubSequences(Sequence seq)
    {
        ...
        while (shouldRunLoop)
        {
            ...
            var subSequence = CalcSubsequence(inputToUse);
